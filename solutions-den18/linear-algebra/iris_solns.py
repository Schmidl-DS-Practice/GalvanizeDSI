...
plt.scatter(width, length, alpha=.5, edgecolor='none')
plt.xlabel('Sepal Width', fontsize=14)
plt.ylabel('Sepal Length', fontsize=14)

means = np.mean(data, axis=0, keepdims=True)
plt.scatter(means[:, :1], means[:, 1:], marker='x', s=300, c='r', alpha=.3, linewidths=5)
...
def check_args(v1, v2):
    if not is_same_shape(v1, v2):
        print("Inputs not the same size")
        raise
    if not is_column_vector(v1):
        print("Not a column vector")
        raise
    return None
...
plt.hist(dist_lst, bins=20, edgecolor='none', density=1)
...
plt.hist(cos_lst, bins=20, edgecolor='none', density=1)
